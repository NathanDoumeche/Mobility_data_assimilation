...
#Orange Data
Data_orange = pd.read_csv('/Data/data_orange.csv')
...
actual_all_hour = merged_orange.loc[:,'Residuals']
...
for j in range(48):
    ...
    variables_to_drop = ['Residuals', 'Load', 'NN', 'Static','Dyn','Em']
    X0 = Data_train.drop(variables_to_drop, axis=1)
    ...
    rmse_train = root_mean_squared_error(Data_train[['Residuals']], y_train_denormalized)
    ...
    variables_to_drop = ['Residuals', 'Load', 'NN', 'Static','Dyn','Em']
    X1 = Data_test.drop(variables_to_drop, axis=1)
    ...
    rmse_test = root_mean_squared_error(Data_test[['Residuals']], y_pred_denormalized)
    ...
    rmse_oracle = root_mean_squared_error(Data_test[['Residuals']], y_pred_denormalized_oracle)
    ...
    y = np.concatenate((Data_train[['Residuals']], Data_test[['Residuals']]))
    ...
    ltest = len(Data_test[['Residuals']].values)
    #### static Kalman 
    ...
    #### dynamic Kalman (EM)
    l_em = expectation_maximization(Xkal, y, Q_init = np.eye(Xkal.shape[1]), p1 = 1, n_iter = 3*10^3)  #######1.8s
...
concatenated_pred.to_csv('/Users/Yannig/Documents/These_Nathan/Mobility_data_review/Results/prediction_MLP_orange_residuals_lag.txt', index=True, header=True)
