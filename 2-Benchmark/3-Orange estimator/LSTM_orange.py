...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 23 15:21:28 2025
...
#MLP prev
Data_prev = pd.read_csv('/Results/prediction_MLP_newdate_lag.txt')
Data_prev = Data_prev.drop(columns=['Load'])
Data_prev['date']= pd.to_datetime(Data_prev['date'])
Data_prev.set_index("date", inplace=True)
Data_prev.index

#Orange Data
Data_orange = pd.read_csv('/Data/data_orange_nathan.csv')
Data_orange['date']= pd.to_datetime(Data_orange['date'])
Data_orange = Data_orange[['date', 'Residents_aggl', 'Work', 'Exc_aggl', 'Tourists_aggl']]
Data_orange.set_index("date", inplace=True)
Data_orange.index
Data_orange.columns.tolist()

#merging
merged_df = pd.merge(Data, Data_prev, left_index=True, right_index=True, how='inner')
merged_df.columns.tolist()
merged_df.index = merged_df.index.tz_localize(None)
merged_orange = pd.merge(merged_df, Data_orange, left_index=True, right_index=True, how='inner')
merged_orange.columns.tolist()
merged_orange['Residuals'] = merged_orange['Load'] - merged_orange['NN']
...
covariate_columns = ['Residuals', 'Exc_aggl']

merged_orange.columns.tolist()
merged_orange = merged_orange[covariate_columns]


a = merged_orange[merged_orange.index.astype(str) == "2019-07-08 00:00:00"].index[0]
actual_all_hour = merged_orange.loc[a:,'Residuals']
...
for j in range(48):
    ...
    Data_h = merged_orange.between_time(start_time, end_time)
    
    #d1 = Data_h[Data_h.index.strftime('%Y-%m-%d %H:%M:%S') ==  "2022-09-01 "+ start_time + ":00"].index[0]
    #d2 = Data_h[Data_h.index.strftime('%Y-%m-%d %H:%M:%S') ==  "2022-09-02 "+ start_time + ":00"].index[0]
    
    d1 = Data_h[Data_h.index.strftime('%Y-%m-%d %H:%M:%S') ==  "2022-09-01 "+ start_time + ":00"].index[0]
    d2 = Data_h[Data_h.index.strftime('%Y-%m-%d %H:%M:%S') ==  "2022-08-26 "+ start_time + ":00"].index[0]


    mean_load_train = np.mean(Data_h.loc[:d1, 'Residuals'])
    std_load_train = np.std(Data_h.loc[:d1,'Residuals'])
   
    mean_load_test = np.mean(Data_h.loc[d2:, 'Residuals'])
    std_load_test = np.std(Data_h.loc[d2:,'Residuals'])
   ...
   # Denormalize predictions and actuals
    ...
    predictions_denormalized_oracle = np.array(predictions) * std_load_test + mean_load_test
    actuals_denormalized = np.array(actuals) * std_load_test + mean_load_test
   
   
   # Calculate hourly RMSE
   
    rmse_base = np.sqrt(np.mean(Data_test[['Residuals']]**2))
    print(f'Residuals on Test Data: {rmse_base:.4f}')
    
    rmse_test = np.sqrt(np.mean((predictions_denormalized - actuals_denormalized)**2))
    print(f'Final Root Mean Squared Error on Test Data: {rmse_test:.4f}')

    rmse_oracle = np.sqrt(np.mean((predictions_denormalized_oracle - actuals_denormalized)**2))
    print(f'ORACLE Root Mean Squared Error on Test Data: {rmse_oracle:.4f}')
    ...
    #### dynamic Kalman (EM)
    l_em = expectation_maximization(Xkal, y, Q_init = np.eye(Xkal.shape[1]), p1 = 1, n_iter = 9*10^3) 
    ...
    print(f'rnn_dyn_em Root Mean Squared Error on Test Data: {rmse_dyn_em:.4f}')

    len(indices_to_update)
    y_rnn.shape
...
concatenated_pred.columns = ['Residuals', 'NN', 'Static', 'Dyn', 'Em']
concatenated_pred.to_csv('/Results/prediction_LSTM_orange_residuals.txt', index=True, header=True)
