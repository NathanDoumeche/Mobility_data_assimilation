...
gb_forecast_elec = predict(gb_cv, Data_nat)


#Loading Orange data
Data_nat$residuals = Data_nat$Load - gb_forecast_elec
...
gb_formula  <- as.formula("residuals~
                            bbs(Exc_rec_aggl, knots=20, df=4) +
                            bbs(Residents_aggl, knots=20, df=4) +
                            bbs(temperature, knots=20, df=4)+
                            bbs(toy, knots=20, df=4) +
                            bols(day_type_week) +
                            bbs(temperature_smooth_950, knots=20, df=4)+
...
gb = gamboost(gb_formula, data = Data_Orange[train_Orange,],
              control = boost_control(nu = 0.15, mstop = 500))
...
gb_cv =  gamboost(gb_formula, data = Data_Orange[train_Orange,], 
                  control = boost_control(nu = 0.15, mstop = mstop))
gb_forecast = predict(gb_cv, Data_Orange)

RMSE(Data_Orange$residuals, gb_forecast, test_Orange)
MAPE(Data_Orange$Load, gb_forecast_elec[which(Data_nat$date %in% Data_Orange$date)]+  gb_forecast, test_Orange)


### Saving
estimators <- cbind(Data_Orange$Load, gb_forecast_elec[which(Data_nat$date %in% Data_Orange$date)], 
                    gb_forecast_elec[which(Data_nat$date %in% Data_Orange$date)]+  gb_forecast)
estimators = estimators[test_Orange,]
colnames(estimators)[c(1,2,3)] = c("Load", "GAM_boosting_elec", "GAM_boosting_Orange")

if(HOLIDAYS)
{
  saveRDS(estimators, "Results/estimatorGamBoostingHolidays.RDS")
}else{
  saveRDS(estimators, "Results/estimatorGamBoostingComplete.RDS")
}
